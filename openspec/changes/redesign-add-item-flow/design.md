...
**6. Save gating + return to idle (same box retained).** `Save item` is gated only on a box (and, unless in none mode, a valid item code); photo and description are both optional (product decision — don't block on a photo). After a successful save the sheet returns to the idle screen and keeps the box as the current target (persisted). It does **not** auto-mint/print the next label: printing only happens on a deliberate **New item** tap (print-on-open), so saving one item never prints the next one unprompted — an earlier auto-loop that pre-printed the next label was reverted as surprising. Retaining the box keeps the next item one tap away without the surprise print.
